...
export default function PastoraisPage() {
  return (
    <div className="mx-auto max-w-6xl px-4 py-12">
      <h1 className="text-4xl text-terracota-escuro">Pastorais e Movimentos</h1>
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {pastorais.map((pastoral) => (
          <article
            key={pastoral.nome}
            className="group flex flex-col rounded-xl border border-dourado-claro bg-white p-6 shadow-sm transition-all hover:-translate-y-1 hover:border-dourado hover:shadow-md"
          >
            <Users className="h-8 w-8 text-terracota transition-colors group-hover:text-dourado" aria-hidden />
            <h2 className="mt-3 text-xl text-marrom">{pastoral.nome}</h2>
            <p className="mt-2 flex-1 text-sm leading-relaxed">
              {pastoral.descricao}
            </p>
            <dl className="mt-4 space-y-1.5 border-t border-creme-escuro pt-4 text-sm text-marrom-claro">
              <div className="flex items-center gap-2">
                <dt className="sr-only">Reuniões</dt>
                <CalendarClock className="h-4 w-4 text-dourado" aria-hidden />
